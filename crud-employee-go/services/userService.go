...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	result, err := statement.Exec(newUser.ID, newUser.Name, newUser.Class, newUser.Legs)
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var user User
		if err := rows.Scan(&user.ID, &user.Name, &user.Class, &user.Legs); handleGenericError(w, "Failed to scan users!", err) {
			return
		}
...
func GetUserByID(w http.ResponseWriter, r *http.Request) {
	...
	row, err := db.Query("SELECT * FROM animal WHERE id = ?", ID)
	...
	var user User
	if row.Next() {
		if err := row.Scan(&user.ID, &user.Name, &user.Class, &user.Legs); handleGenericError(w, "Failed to scan users!", err) {
			return
		}
	}
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	statement, err := db.Prepare("UPDATE animal SET id = ?, name = ?, class = ?, legs = ? WHERE id = ?")
	if handleGenericError(w, "Failed to create statement!", err) {
		return
	}
	defer statement.Close()

	if _, err := statement.Exec(updatedUser.Legs, updatedUser.Class, updatedUser.Name, ID); handleGenericError(w, "Failed to execute statement!", err) {
		return
	}
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	statement, err := db.Prepare("DELETE FROM animal WHERE id = ?")
	if handleGenericError(w, "Failed to create statement!", err) {
		return
	}
	defer statement.Close()
